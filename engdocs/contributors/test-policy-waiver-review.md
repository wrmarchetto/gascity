...
| resourcecensus | `medium package_dir=cmd/gc package_name=main owner=TestReleaseIfCurrentKeepsUnroutedWorkVisibleToPoolDemand` | 2026-11-09 | retain | 2026-09-18 | the demand predicate IS a shell pipeline over `bd` (`poolDemandCountShell`), so only running it can show a released bead is visible to the probe that decides whether a session is minted; a Go re-derivation of the same predicate would agree with itself whichever field the release wrote, which is precisely the producer/consumer mismatch the test exists to catch |
...
| resourcecensus | `medium package_dir=internal/beads/contract package_name=contract owner=TestManagedPortOwnershipSeesARealListenerInThisProcess` | 2026-11-09 | retain | 2026-09-15 | the one case in its file that reads the REAL /proc socket table; the others read a procfs tree the test itself wrote and so agree with the fixture rather than with the kernel's formatting, which is the thing being parsed |
| resourcecensus | `medium package_dir=internal/beads/contract package_name=contract owner=TestValidManagedRuntimeStateSkipsTheDialWhenOwnershipAnswers` | 2026-11-09 | retain | 2026-09-15 | the claim is that a resolvable managed state costs the Dolt server no accept at all, and only a port this process genuinely holds can be put to the socket table that way |
| resourcecensus | `medium package_dir=internal/beads/contract package_name=contract owner=TestValidManagedRuntimeStateStillDialsWhenOwnershipCannotAnswer` | 2026-11-09 | retain | 2026-09-15 | the fallback exists for hosts that cannot attribute the socket, so the dial it makes must have a real listener to reach; retired when the dial fallback is |
