...
//
// Both manifest consumers in this package are covered, because they do not
// scan the same verb set. bdMutationWriteIDs handles update/close/reopen/
// delete; bdPreWriteMutation additionally gates `create`, so a create-only
// flag is reachable through nothing else here.
//
...
//
//	go test ./cmd/gc/ -run 'TestBdMutationScanner|TestBdPreWriteMutation'
...
// TestBdPreWriteMutationAdmitsRealBdFlags pins the same invariant on the
// pre-write validator gate, which is the consumer the gs-9zu report actually
// named. It reaches a verb bdMutationWriteIDs does not scan at all -- `create`
// -- so bd's create-side hidden aliases are covered through nothing else.
//
// A refusal here means the argv never reaches the city's validator OR bd: the
// write is rejected outright, which is the behavior gs-9zu describes.
func TestBdPreWriteMutationAdmitsRealBdFlags(t *testing.T) {
	cases := []struct {
		name string
		args []string
	}{
		{
			name: "update --if-assignee, the argv from the gs-9zu report",
			args: []string{"update", "gs-9zu", "--assignee", "toolsmith-1", "--if-assignee", "toolsmith-1"},
		},
		{
			// The report's exact shape: a global -C sitting AFTER the verb.
			name: "update --if-assignee behind a post-verb -C",
			args: []string{"update", "-C", "/home/willie/gascity", "gs-9zu", "--assignee", "toolsmith-1", "--if-assignee", "toolsmith-1"},
		},
		{
			name: "update --if-status",
			args: []string{"update", "gs-9zu", "-s", "open", "--if-status", "in_progress"},
		},
		{
			name: "create with the hidden -m description alias",
			args: []string{"create", "-m", "a description", "--title", "t"},
		},
		{
			name: "create with the hidden --body description alias",
			args: []string{"create", "--body", "a description", "--title", "t"},
		},
		{
			name: "create --label",
			args: []string{"create", "--label", "harness:x", "--title", "t"},
		},
		{
			name: "create --allow-empty-description is boolean",
			args: []string{"create", "--allow-empty-description", "--title", "t"},
		},
		{
			name: "close with the hidden --resolution alias",
			args: []string{"close", "gs-9zu", "--resolution", "done"},
		},
		{
			name: "a global value flag before the verb",
			args: []string{"--actor", "toolsmith-1", "update", "gs-9zu", "--if-assignee", "toolsmith-1", "-a", "x"},
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !mutation {
				t.Fatalf("bdPreWriteMutation(%q): mutation = false, want true; the verb was not recognized, so no validator sees this write", tc.args)
			}
			if ambiguous {
				t.Fatalf("bdPreWriteMutation(%q): refused as ambiguous; internal/bdflags is missing a flag bd accepts, so gc rejects the write before bd or the validator runs", tc.args)
			}
		})
	}
}

// TestBdPreWriteMutationStillRefusesUnknownFlags is the fail-closed half for
// the validator gate. Its refusal exists so a validator is never handed an
// argv it would misread -- a shifted positional makes a label or metadata edit
// look like it belongs to a different bead -- so completing the manifest must
// not soften it.
func TestBdPreWriteMutationStillRefusesUnknownFlags(t *testing.T) {
	cases := [][]string{
		{"update", "--if-owner", "toolsmith-1", "gs-9zu", "-a", "x"},
		{"create", "--no-such-flag", "value", "--title", "t"},
		{"close", "--unknown-future-flag", "gs-realbead", "gs-9zu"},
	}
	for _, args := range cases {
		mutation, ambiguous := bdPreWriteMutation(args)
		if !mutation {
			t.Errorf("bdPreWriteMutation(%q): mutation = false, want true", args)
			continue
		}
		if !ambiguous {
			t.Errorf("bdPreWriteMutation(%q): ambiguous = false, want true; an unknown flag was admitted and the validator may be handed an argv whose positionals have shifted", args)
		}
	}
}
