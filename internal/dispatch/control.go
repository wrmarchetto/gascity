...
// the next iteration or exhausts. A gate that could not run at all
// (GateError/GateTimeout) is neither: it returns ErrControlPending for a
// re-run that costs no attempt, until the infra-retry budget is spent.
