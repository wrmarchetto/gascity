...
// TestScopeCheckPassPropagatesOutputJSONToRalphBodyScope pins that closing a
// ralph body scope carries the member's gc.output_json up onto the scope bead,
// which is what makes a loop body's output readable by the steps after it.
//
// This test used to continue into a second half that dispatched a
// gc.kind=check bead and asserted the same value reached the logical bead.
// That half was removed with the kind (ci-zg0l), not rewritten against the
// live path: the assertions kept here are the ones that were already running
// against live code. The absent half is deliberate -- the ralph control now
// owns iteration outcomes directly, so there is no separate check bead to
// propagate through.
func TestScopeCheckPassPropagatesOutputJSONToRalphBodyScope(t *testing.T) {
	t.Parallel()
