...
### Turn end (`Stop`) is deliberately not wired

Claude Code exposes `Stop` and `gc` ships a gate for it -- `gc hook stop`
refuses to end a turn while the session's claimed work is unclosed. The
default template still does not wire it. A closing contract belongs to one
deployment's prompts, and this file is `//go:embed`-ed into the binary and
written into every city `gc` installs, so wiring it here would ship one
deployment's contract to cities that have no such contract and no
`gc hook stop` expectation.

A city that wants the gate puts a `Stop` block in its own
`<city>/.claude/settings.json`, which `desiredClaudeSettings`
(`internal/hooks/hooks.go`) merges over these defaults -- that city gains the
gate and keeps inheriting every default added here later.
`TestInstallClaudeMergesCityOverrideAddingANewHookEvent` pins both halves.
Re-adding it here to save one city that file puts it back on every city's
turn boundary.
