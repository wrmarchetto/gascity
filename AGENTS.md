...
This is a deliberately diverged Gas City fork. Per the rig's PM decision
recorded in `docs/pm-log.md` #1, favor the city's requirements over routine
upstream alignment; only consider an official upstream release when a task
specifically needs it. T3 Code and the DoltLite beads backend are context, not
live dependencies for ordinary work.

When a feature appears missing or regressed, inspect the relevant git history
before rebuilding it. Prefer the smallest proven slice, confirm archived plans
and audits against current code, and keep provider-specific assumptions behind
their existing runtime, config, or beads boundaries.
