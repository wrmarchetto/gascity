...
| `claim_routes` | []string |  |  | ClaimRoutes optionally permit this agent to claim new unassigned work routed to shared queues. They supplement the agent's own qualified pool route, while assigned-work recovery remains restricted to the session's own identity. Supports the same Go template placeholders as WorkQuery. |
