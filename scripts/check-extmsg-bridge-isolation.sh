...
# The package's prose is collected separately because the two scans want
# different sets. Nothing imports a README, so the closure holds source only,
# and that is the right scope for the role scan: criterion 5 forbids a role
# name in SOURCE, while a README naming one in a SLACK_TARGET_AGENT example is
# the criterion being satisfied -- it shows the identity arriving as
# configuration. Refusal 4 wants the opposite and adds the prose back, because
# documentation pointing an operator at the alerts channel unifies the two
# channels just as effectively as code that reads it.
# --- two scopes, because the refusals ask two different questions ---
#
# PKG_FILES above is everything the package can RUN. Refusals 1 and 4 use it
# whole: a role name or an alerts-seam reference is forbidden anywhere the
# bridge ships, launchers and stand-ins included, because that text binds an
# identity or a destination wherever it sits.
#
# MODULE_FILES is the narrower question -- the bridge's own ESM module graph,
# which is the .mjs half of the same closure. Refusals 2 and 3 are about HOW
# the bridge reads its configuration, and their remedy is a helper
# (lib/gc-client.mjs's env()) that only a module importing it can use. The
# extensionless stand-ins fake-imsg/imsg and fake-telegram/bot-api are
# separate programs spawned by demo.sh: they load node builtins alone, read
# only their own FAKE_* knobs, and so cannot bind a session or a channel.
# Refusing their `process.env.FAKE_TG_PORT || 8932` would be refusing
# something that is not a violation, and the fix would be to import a bridge
# module into a stand-in -- worse than the thing refused. The extension is
# the derivation, NOT a list of exempt filenames: a stand-in rewritten as a
# .mjs and imported is in the narrow scope the moment it is.
MODULE_FILES=()
...
if [ "${#SRC_FILES[@]}" -gt 0 ]; then
    hits=$(grep -rniE "$ROLE_PATTERN" -- "${SRC_FILES[@]}" || true)
    if [ -n "$hits" ]; then
        echo "$hits" >&2
